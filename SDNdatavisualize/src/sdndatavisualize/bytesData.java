...
public class bytesData {
    public void create_file(String query, String query1) throws IOException{
        try{
            ...
            while (rs.next())
            {
              ...
              int transmit = rs.getInt("transmitted_bytes");
              int receive = rs.getInt("received_bytes");
            ...
            new bytesData().create_json(mac_arr,receive_arr,transmit_arr,count_row);
            st1.close();
            st.close();
    ...
    public void create_json(String[] mac, int[] receive, int[] transmit, int rows) throws IOException{
        ...
        try(FileWriter file = new FileWriter("/Users/HP/Desktop/kibana/bytes/bytedata.json")){
            for(int i=0; i<rows; i++){
                String number = Integer.toString(i+1);

                JSONObject obj = new JSONObject();
                obj.put("_index", "bytedata");
                obj.put("_id", number);
                ...
                obj_1.put("mac", mac[i]);
                obj_1.put("number_of_receive_bytes", receive[i]);
                obj_1.put("number_of_transmit_bytes", transmit[i]);
